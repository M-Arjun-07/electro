...
import { Sword, MessageSquare, Zap, MapPin, ChevronRight, Award } from 'lucide-react';

export default function Dashboard() {
  const { username, userProfile, getOverallKnowledge } = useContext(AppContext);
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto flex flex-col gap-8 pb-24 md:pb-8">
      {/* Knowledge Bar */}
      <div className="bg-white dark:bg-darkSurface p-5 md:p-6 rounded-3xl border border-slate-200 dark:border-zinc-800 shadow-xl flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden relative">
      ...
      {/* Hero Section */}
      <div className="bg-gradient-to-br from-primary via-indigo-700 to-indigo-900 rounded-[2.5rem] p-6 md:p-12 text-white shadow-2xl relative overflow-hidden flex flex-col md:flex-row justify-between items-center gap-8 border border-white/10">
         ...
         <div className="z-10 text-center md:text-left w-full md:w-auto">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-2 md:gap-3 mt-4">
               <span className="px-3 md:px-4 py-2 bg-black/20 rounded-xl text-[10px] md:text-xs font-bold border border-white/10 backdrop-blur-sm whitespace-nowrap">Level {Math.floor((userProfile.points || 0) / 100) + 1} Citizen</span>
               <span className="px-3 md:px-4 py-2 bg-black/20 rounded-xl text-[10px] md:text-xs font-bold border border-white/10 backdrop-blur-sm whitespace-nowrap">{userProfile.state}</span>
         ...
         <div className="z-10 bg-white/5 backdrop-blur-xl p-6 md:p-8 rounded-[2rem] border border-white/10 text-center min-w-[160px] md:min-w-[200px] shadow-2xl relative group w-full md:w-auto">
            ...
            <span className="text-5xl md:text-6xl font-black text-white drop-shadow-lg">{userProfile.points || 0}</span>
         </div>
      </div>

      {/* Journey Hub CTA */}
      <div className="bg-white dark:bg-darkSurface p-8 rounded-[2.5rem] border border-slate-200 dark:border-zinc-800 shadow-xl relative overflow-hidden group">
